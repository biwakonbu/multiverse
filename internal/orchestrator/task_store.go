...
type SuggestedImpl struct {
	Language    string   `json:"language,omitempty"`    // 推奨言語 (go, typescript, etc)
	FilePaths   []string `json:"filePaths,omitempty"`   // 変更/作成すべきファイルパスのリスト
	Constraints []string `json:"constraints,omitempty"` // 実装上の制約事項
}

// Artifacts represents the outputs generated by the task execution.
type Artifacts struct {
	Files []string `json:"files,omitempty"` // 生成・変更されたファイルのパス
	Logs  []string `json:"logs,omitempty"`  // 関連するログファイルのパス
}
