...
- 【受け入れ条件】
  - **ログの追従と停止**: ログ発生時は自動スクロール（Tail）し、ユーザーがスクロールアップしたら自動スクロールを停止する（Terminal like behavior）。
  - **表示制限**: パフォーマンス維持のため、1000 行を超えたら古いログから破棄する（FIFO）。
  - **操作性**: 「Clear」ボタンでログを消去できる。タスクノードクリック時、当該タスクのログのみにフィルタリング表示される（未選択時は全ログまたは直近のアクティブログ）。
  - **視認性**: `stdout` / `stderr` / `system` (Orchestrator からの通知) が色分けやアイコンで識別できる。
...
- 【受け入れ条件】
  - **Pre-flight Check**: `agent-runner` 起動前に、必要な CLI ツール（`codex` 等）がパスにあり、ログイン状態（`CODEX_SESSION_TOKEN` 等の必須環境変数）が満たされているか検証する。
  - **エラー誘導**: 検証失敗時、単なるエラーログではなく、UI 上で「設定が必要です」といったアクション可能なメッセージを表示する。
  - **セキュアな注入**: セッション情報は環境変数 `CODEX_SESSION_TOKEN` 経由でのみ注入し、ログや設定ファイルには保存しない。
