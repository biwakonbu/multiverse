...
- **言語**: 会話・コメント・ドキュメント・コミットメッセージは全て **日本語** で行う。コード（関数・変数名）は英語。
- **CLI セッション**: LLM 実行には原則として API キーではなく、ローカルの CLI サブスクリプション（Codex, Claude Code 等）を使用する設計を優先してください。
...
- **安全ガイドライン**: テスト失敗時に `git checkout -- .` や類似コマンドで既存実装を消さない。実装は誰かの作業成果である可能性が高いため、指示を確認し、テスト修正や原因調査を優先すること。破壊は修正ではない。
